...
export function createRoomGeometryBuilder({
  ...
  function addOriginElevator(
    ...
    if (typeof registerElevatorCarDescriptor !== 'function') {
      return;
    }
    ...
    const indicatorMinX = centerX + halfSize - railThickness + indicatorDepth * 0.2 - trimMargin;
    ...
    registerElevatorCarDescriptor({
      ...
      colors: {
        platform: mixColors(profile.floorColor, profile.accentColor, 0.45),
        trim: mixColors(profile.floorColor, profile.ceilingColor, 0.4),
        rail: mixColors(profile.wallColor, profile.accentColor, 0.55),
        post: mixColors(profile.wallColor, profile.ceilingColor, 0.55),
        canopy: mixColors(profile.ceilingColor, profile.accentColor, 0.55),
        indicator: mixColors(profile.accentColor, profile.ceilingColor, 0.5)
      }
    });
  }
  ...
  function buildGeometryForCenter(cx, cz) {
    ...
    if (typeof resetElevatorCarDescriptor === 'function') {
      resetElevatorCarDescriptor();
    }
