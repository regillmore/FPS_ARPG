...
export function createRoomGeometryBuilder({
  ...
  function buildGeometryForCenter(cx, cz) {
    ...
    function shouldSkipSolidWallBetweenRooms(ax, az, bx, bz, layerIndex) {
      ...
      const shouldOpenBalconyEdge = balconyA || balconyB;

      if (shouldOpenBalconyEdge) {
        const dx = bx - ax;
        const dz = bz - az;

        if (dx === 1 && dz === 0) {
          if (edgesA?.east === 'solid') edgesA.east = 'open';
          if (edgesB?.west === 'solid') edgesB.west = 'open';
        } else if (dx === -1 && dz === 0) {
          if (edgesA?.west === 'solid') edgesA.west = 'open';
          if (edgesB?.east === 'solid') edgesB.east = 'open';
        } else if (dx === 0 && dz === 1) {
          if (edgesA?.south === 'solid') edgesA.south = 'open';
          if (edgesB?.north === 'solid') edgesB.north = 'open';
        } else if (dx === 0 && dz === -1) {
          if (edgesA?.north === 'solid') edgesA.north = 'open';
          if (edgesB?.south === 'solid') edgesB.south = 'open';
        }
      }
